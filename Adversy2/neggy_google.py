...
class GoogleNeggyAgent: # Renamed class
    ...
    def investigate(self, name: str):
        ...
        self._log(f"I am conducting a comprehensive Google search for '{name}' and will report any adverse findings.")
        
        # This agent performs the broad search and analysis
        all_results = self.search_client.search(name)
        
        if not all_results:
            self._log("[blue]My Google search is complete. No articles of any kind were found.[/blue]")
            return [], []
        ...
        if not adverse_hits:
            self._log(f"[blue]My Google search is complete. No direct adverse media hits found among the {len(all_results)} results.[/blue]")
        
        adverse_hits.sort(key=lambda x: x["risk_score"], reverse=True)
        
        return adverse_hits, general_news

    def run(self, name: str, client: GoogleSearchClient = None):
        """
